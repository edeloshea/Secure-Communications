...
if __name__ == "__main__":
    ...
    k = random_with_N_digits(64)
    
    cipher = SpeckCipher(k, 256, 128, 'ECB')

    ## set the variable that we will use for the testing and evaluation of the simon cipher
    ## for the purposes of this test we will be using 'Edel' as the string
    print ("Testing the Speck crypotgraphic algorithm")
    print ()

    print ("'Edel' will be used as the test word for the encryption and decryption")

    import codecs
    ## get the hex value for 'Edel'
    ...
    print()
    print("Encryption")
    ## input the hex value for 'Edel' into the Simon encrypt function
    ciphertext = cipher.encrypt(0x4564656c)
    ...
    print ("The decrypted ciphertext is " + str(plaintext))

    ## decode the decrypted ciphertext
    print ("The value is returned as an integer. It must be converted to hex and then bytes to get the plaintext")
    decoded_plaintext = hex(plaintext)
    hex_string = decoded_plaintext[2:]
    rr = bytes.fromhex(hex_string)
    print ("The plaintext is " + rr.decode())
