...
if __name__ == "__main__":
    ...
    k = random_with_N_digits(128)

    w = SimonCipher(k, key_size=256, block_size=128)

    import codecs

    ll = (codecs.encode(b"Edel Edel Edel", "hex")).decode()
    print ("The hex for 'Edel Edel Edel' is 0x" + ll)

    print ()
    print ("ENCRYPTION")
    print ("Same plaintext and key")
    for i in range(1,6):
        ciphertext = w.encrypt(0x4564656c204564656c204564656c)
        print ("Ciphertext " + str(i) + " is " + str(ciphertext))

    print ()
    print ("Same plaintext with different keys")
    for i in range(6,11):
        l = random_with_N_digits(128)
        x = SimonCipher(l, key_size=256, block_size=128)
        ciphertext = x.encrypt(0x4564656c204564656c204564656c)
        print ("Ciphertext " + str(i) + " is " + str(ciphertext))
